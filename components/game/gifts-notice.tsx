...
/** "A friend sent medicine to your creature" notice, dismissed once seen. */
    ...
    <div role="status" className="rounded-2xl border border-health/40 bg-health/10 px-4 py-3 text-sm text-cream-100 animate-rise">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-semibold">Un coup de pouce pour {creatureName} !</p>
          <ul className="mt-1 space-y-0.5 text-cream-300">
            {gifts.map((gift) => (
              <li key={gift.id}>
                <strong className="text-cream-100">{gift.from.username}</strong> t&apos;a envoyé un {SHOP_ITEMS[gift.item].label.toLowerCase()}.
              </li>
            ))}
          </ul>
        </div>
