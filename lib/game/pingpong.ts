...
export type PingPongEffect = { kind: "serve" | "hit" | "miss" | "point"; player: string; quality?: HitQuality; reason?: PointReason; at: number };
...
export type HitOutcome = { ok: true; quality: HitQuality; offsetMs: number } | { ok: false; reason: "phase" | "not_receiver" | "early" | "late" };

const MAX_RECENT = 8;

/** Flight time (ms) after `pace` consecutive hits. */
export function flightMs(pace: number): number {
  return Math.max(PINGPONG.minFlightMs, Math.round(PINGPONG.firstFlightMs * PINGPONG.paceFactor ** Math.max(0, pace)));
}
...
export function createPingPong(players: [string, string], now: number, endsAt = now + PINGPONG.maxSeconds * 1000): PingPongState {
...
function launch(state: PingPongState, from: string, now: number, pace: number, quality: Flight["quality"]) {
  state.flight = { id: state.nextFlightId++, from, to: other(state, from), launchedAt: now, arrivesAt: now + flightMs(pace), pace, quality };
  state.phase = "flight";
...
export function servePingPong(state: PingPongState, userId: string, now: number): boolean {
  ...
  launch(state, userId, now, 0, "serve");
...
function awardPoint(state: PingPongState, to: string, reason: PointReason, now: number) {
  ...
  if (state.points[to] >= PINGPONG.pointsToWin) {
    state.phase = "over";
    state.winnerId = to;
    state.nextServeAt = null;
  } else {
 ...
 * (faster after a perfect hit), too early is a point for the sender, too
 * late is ignored (the tick already called the miss).
 */
export function hitPingPong(state: PingPongState, userId: string, now: number): HitOutcome {
  ...
  if (offsetMs < -PINGPONG.goodMs) {
    awardPoint(state, flight.from, "early", now);
    note(state, { kind: "miss", player: userId, reason: "early", at: now });
    return { ok: false, reason: "early" };
  }
  if (offsetMs > PINGPONG.goodMs) return { ok: false, reason: "late" };
  const quality: HitQuality = Math.abs(offsetMs) <= PINGPONG.perfectMs ? "perfect" : "good";
  ...
  launch(state, userId, flight.arrivesAt, flight.pace + (quality === "perfect" ? 2 : 1), quality);
  note(state, { kind: "hit", player: userId, quality, at: now });
  return { ok: true, quality, offsetMs };
}
...
export function tickPingPong(state: PingPongState, now: number, options: TickOptions = {}): void {
  ...
  if (state.phase === "serve" && state.serveDueAt !== null && now >= state.serveDueAt) {
    launch(state, state.server, now, 0, "serve");
  ...
  if (state.phase === "flight" && state.flight) {
    ...
    if (now > state.flight.arrivesAt + PINGPONG.goodMs + grace) {
...
export function inHitWindow(flight: Flight, now: number): boolean {
  return Math.abs(now - flight.arrivesAt) <= PINGPONG.goodMs;
}
...
export function parsePingPongState(raw: unknown): PingPongStateMessage | null {
  ...
  const state = { ...(s as unknown as PingPongState), flight: (s.flight as Flight | undefined) ?? null, recent: [] };
