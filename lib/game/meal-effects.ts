...
export type MealEffects = {
  ...
  full: boolean;
  /** Mood multiplier already applied to `xpDelta` (1 when no mood was given). */
  xpMultiplier: number;
};
...
export function mealEffects(input: { score: number; tier: Tier; hunger: number; mood?: number; rules?: GameRules }): MealEffects {
  const rules = input.rules ?? DEFAULT_RULES;
  const threshold = rules.tiers[input.tier].healthyScoreThreshold;
  ...
  const baseXp = FEEDING.xpGain + (healthy ? FEEDING.xpBonusHealthy : 0);
  return {
    healthDelta: round1(full ? clamped / 2 : clamped),
    hungerDelta: -FEEDING.hungerReduction,
    moodDelta: FEEDING.moodGain,
    xpDelta: input.mood === undefined ? baseXp : applyMoodToXp(baseXp, input.mood, rules.mood),
    healthy,
    full,
    xpMultiplier: input.mood === undefined ? 1 : moodXpMultiplier(input.mood, rules.mood),
  };
}
