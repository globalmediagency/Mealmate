...
async function drawnRectangles(bytes: Uint8Array): Promise<{ x: number; y: number; w: number; h: number }[]> {
  ...
  const streams = contents instanceof PDFRawStream ? [contents] : (contents?.asArray() ?? []).map((ref) => doc.context.lookup(ref, PDFRawStream));
...
describe("markerPdf", () => {
  it("produces a one-page PDF", async () => {
    ...
    expect(doc.getTitle()).toContain("MealMate");
  ...
  it("prints exactly the detector's marker, 8 cm wide, top row at the top of the page", async () => {
    const rects = await drawnRectangles(await markerPdf());
    const mm = 72 / 25.4;
