...
import { SpeciesToggle } from "@/components/admin/species-toggle";
...
export default async function AdminCreaturesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [dropWeights, disabled] = await Promise.all([getDropWeights(), getDisabledSpecies()]);
    ...
    <div className="space-y-6 animate-rise">
      <div>
        ...
        <p className="mt-1 text-sm text-cream-500">
          ...
          L&apos;interrupteur retire une espèce des tirages : ses propriétaires la gardent, elle ne sort simplement plus des œufs
          {disabled.size > 0 ? ` (${disabled.size} désactivée${disabled.size > 1 ? "s" : ""})` : ""}.
        </p>
      </div>
        ...
        <div className="space-y-6">
          {detailed.map((s) => (
            <SpeciesSheet
              key={s.id}
              id={s.id}
              closeHref={showAll ? undefined : href(keep)}
              enabled={!disabled.has(s.id)}
              chance={disabled.has(s.id) ? "désactivée" : formatChance(speciesWeights(s.tier, dropWeights.species, disabled).find((w) => w.item.id === s.id)!)}
            />
          ))}
        </div>
      ) : null}
      ...
      {tiers.map((t) => {
        const weighted = speciesWeights(t, dropWeights.species, disabled);
          ...
          <section key={t} className="space-y-3">
            ...
            <ul className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
              {list.map((s) => {
                ...
                return (
                  <li key={s.id} className={cn("relative flex flex-col rounded-2xl border bg-ink-800/80", selected?.id === s.id ? "border-sage-400" : "border-ink-600/80", off && "border-dashed")} data-species-card={s.id} data-enabled={off ? "false" : "true"}>
                    <Link href={href({ ...keep, species: s.id })} className="flex flex-1 flex-col items-center gap-1 rounded-t-2xl p-2 text-center transition-colors hover:bg-ink-700/60">
                      <Creature species={s} stage="adulte" state="healthy" size={88} animated={false} className={off ? "opacity-40 grayscale" : undefined} />
                      <span className="truncate text-sm font-semibold text-cream-50">{s.name}</span>
                      <RarityBadge rarity={s.rarity} className="text-[10px]" />
                      <span className="text-[10px] tabular-nums text-cream-500">{off ? "ne sort plus des œufs" : chanceOf.get(s.id) ? formatChance(chanceOf.get(s.id)!) : ""}</span>
                    </Link>
                    <SpeciesToggle speciesId={s.id} name={s.name} enabled={!off} compact className="border-t border-ink-600/60 px-1 py-1.5" />
            ...
            <DropRateEditor
              ...
              rows={weighted.map((w) => ({ id: w.item.id, name: w.item.name, rarity: w.item.rarity, defaultWeight: w.defaultWeight, weight: w.weight, overridden: w.overridden, disabled: w.disabled }))}
            />
          </section>
        );
      })}
...
function SpeciesSheet({ id, closeHref, chance, enabled }: { id: string; closeHref?: string; chance?: string; enabled: boolean }) {
  const s = getSpecies(id);
  if (!s) return null;
  return (
    <section className="rounded-3xl border border-sage-500/40 bg-ink-800/80 p-4 shadow-card" data-species-sheet={s.id}>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <SpeciesToggle speciesId={s.id} name={s.name} enabled={enabled} />
      ...
      <p className="mb-3 text-sm text-cream-300">
        ...
        {chance ? (
          <span className="block text-xs text-cream-500">
            Chance d&apos;éclosion : {chance}
            {!enabled ? " · les joueurs qui l'ont déjà la gardent" : ""}
          </span>
        ) : null}
      </p>
