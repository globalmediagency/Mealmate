...
- `components/ar/ar-viewer.tsx` reçoit `targets: ArTarget[]` (numéro → créature, `mine`, `ownerName`) : caméra, boucle de détection sur une copie réduite de l'image, une boîte de rendu par marqueur reconnu (étiquette nom · propriétaire), positionnée par `style.transform` (l'état React ne change qu'à l'apparition ou la disparition d'un marqueur), photo composée de toutes les créatures visibles, arrêt quand l'onglet est caché. Le rendu est un `ArRendererProps` (`components/ar/types.ts`) : `ViewsRenderer` (niveau 2, faux 3D à 8 vues) aujourd'hui, qui passe `yaw = view × 45°` au composant `Creature` ; `view` vient de `viewFromAngle(pose.angle, précédent)` (`lib/creatures/turnaround.ts`, hystérésis de 6°, état React mis à jour seulement au changement de vue) ; le niveau 3 (Three.js) lira la pose complète (`vendor/posit1.js` fournit POSIT). Changer le marqueur = changer `AR_MARKER` : PDF, aperçu et détecteur suivent, `lib/ar/pdf.test.ts` vérifie que le PDF reproduit la grille du détecteur.
- **Faux 3D (tour d'horizon)** : `Creature` accepte `yaw` (degrés, 0 = face, positif = sens horaire vu de dessus, le visage glisse vers la gauche du spectateur). `turnPlacements(layout, yaw)` (`components/creatures/turn.ts`) projette chaque trait sur une sphère (`turnFeature` : x, `squash`, visibilité, profondeur) : yeux et joues par côté (l'œil du fond disparaît au profil, tout le visage de dos), bouche et lunettes (cachées de dos), oreilles (jamais cachées, retournées quand elles changent de côté, largeur ≥ 45 %), queue (passe devant le corps quand `depth > 0,1`, miroir à gauche), chapeau et collier décalés, accessoire de corps : couche `back` devant le corps quand `facing < 0`. `yaw = 0` donne exactement le rendu d'avant. Contrôle visuel : `/dev/screens?screen=turnaround` (5 silhouettes × 8 vues).
- Test sans caméra : `/dev/screens?screen=ar` affiche le viewer avec deux cibles (n° 17 et 42) et leurs aperçus ; Chromium accepte une fausse caméra (`--use-fake-device-for-media-stream --use-file-for-fake-video-capture=<fichier.mjpeg>`) construite à partir d'une capture de cet aperçu.
