...
- **Détection** : js-aruco2 (MIT, intégré en modules ES dans `lib/ar/vendor/`, dictionnaire AprilTag BSD, chargé à la demande) analyse chaque image réduite à 360 px de large ; chaque marqueur reconnu donne quatre coins, convertis en `MarkerPose` (`lib/ar/geometry.ts` : centre, taille apparente, angle dans le plan, inclinaison, étendues) avec un lissage exponentiel. Tout reste sur le téléphone : aucune image n'est envoyée.
- **Niveaux de rendu** (`components/ar/types.ts`, `ArRendererProps`) : niveau 1 (dépassé) = la créature plate toujours face à la caméra ; **niveau 2 `ViewsRenderer` (livré)** = faux 3D à **8 vues** (45° chacune) : la rotation du papier sur la table (`pose.angle`, angle du bord haut du marqueur à l'écran) choisit la vue avec une hystérésis de 6° (`viewFromAngle`), et le rendu passe `yaw = vue × 45°` au composant `Creature`. Aucun dessin supplémentaire : `turnPlacements()` projette les traits existants sur une sphère (`turnFeature`) : les yeux, joues, bouche et lunettes glissent vers le côté, se resserrent et disparaissent derrière la silhouette (un seul œil au profil, aucun trait de dos), les oreilles se décalent et se retournent, la queue tourne autour du corps et passe devant lui de dos, chapeau et collier suivent, la cape se voit de dos. Mise à l'échelle par la taille du marqueur (hauteur ≈ 2,2 × le carré), ombre au sol suivant l'étendue du marqueur ; niveau 3 « 3D » = modèle glTF rendu par Three.js à partir de la pose complète (`lib/ar/vendor/posit1.js` fournit déjà l'estimation POSIT). Le viewer positionne la boîte du rendu chaque image via `style.transform`, jamais par l'état React.
- **Photo** : le bouton « Photo » compose l'image de la caméra et toutes les créatures visibles (SVG sérialisés, les plus lointaines d'abord) dans un canvas ; enregistrement (`<a download>`) ou partage natif (`navigator.share`) quand le navigateur le propose. La caméra est coupée quand l'onglet passe en arrière-plan.
