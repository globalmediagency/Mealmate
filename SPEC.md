...
- **Arbitrage** : le serveur tient la vérité (santé, bons aliments, événements, fin, rangs, récompenses réservées par `UPDATE` conditionnel), calculée paresseusement à chaque lecture (`snapshot`). La géométrie (où tombe l'œuf, ce que balaie la langue) est jugée par le téléphone du tireur, seule sa caméra sachant où sont les feuilles ; le serveur impose la cadence (un œuf toutes les 250 ms au plus), les dégâts et le plafond de soin. Synchronisation par **sondage court** (`GET /api/arena/:id?since=` toutes les 500 ms en bataille, 2 s en salle, rien une fois la partie finie) ; la case admin « WebRTC » enregistre le souhait d'une liaison directe entre téléphones, à brancher plus tard derrière la même interface `ArenaTransport` (le client retombe sur le sondage tant qu'elle n'existe pas).
...
- `POST /api/arena/:id/action { action: join | decline | start | leave | cancel }` → instantané ; `{ action: "shoot", targetUserId, x, y, hit }` → `{ accepted, hit, targetHp, eliminated }` ; `{ action: "eat", bonusIds, angle, length }` → `{ eaten, healed, hp }`.
