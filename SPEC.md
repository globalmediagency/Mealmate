...
- **Niveaux de rendu** (`components/ar/types.ts`, `ArRendererProps`) : niveau 1 `BillboardRenderer` (livré) = la créature plate posée comme une figurine en carton, toujours face à la caméra, mise à l'échelle par la taille du marqueur (hauteur ≈ 2,2 × le carré), ombre au sol suivant l'étendue du marqueur ; niveau 2 « vues » = plusieurs dessins par espèce choisis d'après `pose.angle` quantifié (`view`, mis à jour seulement quand il change) ; niveau 3 « 3D » = modèle glTF rendu par Three.js à partir de la pose complète (`lib/ar/vendor/posit1.js` fournit déjà l'estimation POSIT). Le viewer positionne la boîte du rendu chaque image via `style.transform`, jamais par l'état React.
