...
- **Marqueur** : ArUco `ARUCO_MIP_36h12` n° 17 (`lib/ar/config.ts`), le même pour tout le monde : le papier ne porte aucune donnée, il donne seulement une position à la caméra. `GET /api/ar/marker` renvoie une page A4 (pdf-lib) avec le carré de 8 cm, sa marge blanche, une ligne de découpe et les consignes ; la page `/ar` montre aussi un aperçu SVG (`markerSvg()`). Le test `lib/ar/pdf.test.ts` relit les rectangles du PDF et vérifie qu'ils reproduisent exactement la grille du détecteur (`markerCells()`), dans le bon sens et à la bonne taille.
- **Détection** : js-aruco2 (MIT, intégré en modules ES dans `lib/ar/vendor/`, chargé à la demande) analyse chaque image réduite à 360 px de large ; le marqueur reconnu donne quatre coins, convertis en `MarkerPose` (`lib/ar/geometry.ts` : centre, taille apparente, angle dans le plan, inclinaison, étendues) avec un lissage exponentiel. Tout reste sur le téléphone : aucune image n'est envoyée.
...
- **Photo** : le bouton « Photo » compose l'image de la caméra et la créature (SVG sérialisé) dans un canvas ; enregistrement (`<a download>`) ou partage natif (`navigator.share`) quand le navigateur le propose. La caméra est coupée quand l'onglet passe en arrière-plan.
...
| `creatures` | Œuf → vivante → morte | `status ∈ {egg, alive, dead}`, `tier`, `species_id`, `rarity`, stats (`health`, `hunger`, `mood` en double precision, `xp` entier), `sick_since`, `protected_until`, `last_tick_at`, mort (`died_at`, `death_cause`, `lifespan_days`), `mourned_at` (migration 002), `accessory_drops` (migration 004), `chest_bonus_steps` (migration 012). Index `(user_id, status)` + **index unique partiel** `user_id WHERE status IN ('egg','alive')` (une seule créature active) |
