...
- Règles (`lib/game/rules.ts`) : par niveau `hatchSteps`, `healthyScoreThreshold`, `hungerPerHour`, `healthLossPerHourWhenStarving`, `moodLossPerHour`, `sickDaysBeforeDeath`, `boardingMaxDays` ; communes `hungerDamageThreshold`, `tick.fullRateHoursCap`, `tick.slowRate`, `feeding.maxMealsPerDay`, `feeding.rejectScreenPhotos`, `feeding.mealRetentionDays`, `boarding.maxPerHost`, `boarding.cooldownMultiplier`, `coaching.thumbsPerStudentReward`, `coaching.thumbsPerCoachReward`, `mood.*`, `defense.*`, `play.maxPerDay` (parties par jour et par créature, « Jouer » et « Défendre » confondus). Validation zod avec bornes, fusion sur les défauts (`DEFAULT_RULES` = constantes de `config.ts`).
...
- **Onglets** (`components/admin/admin-tabs.tsx`) : Règles de jeu, Créatures (`/admin/creatures` : les 60 espèces triées par niveau puis rareté, chacune avec un bouton qui l'ouvre **en 3D** dans une boîte de dialogue, le volume de « Voir en vrai » tourné du doigt à 360° avec le stade et l'état au choix, filtres niveau / rareté, fiche dépliable avec les 4 stades × 4 états et la silhouette, ou toutes les fiches d'un coup) et Accessoires (`/admin/accessoires` : les 30 accessoires filtrables par emplacement, avec rareté, icône et aperçu porté par une espèce de référence à chaque stade). Le catalogue vit dans le code ; seules les **probabilités** se règlent.
- **Probabilités d'obtention** (`lib/game/drops.ts`, ligne `game_settings` id `drops`) : chaque espèce et chaque accessoire a un poids en pourcentage du groupe (jusqu'à 3 décimales, soit 0,001 %). Défaut = part de sa rareté (60 / 28 / 10 / 2 % pour les espèces, 65 / 25 / 8 / 2 % pour les accessoires) répartie également dans la rareté, soit par exemple 6,667 % (1 chance sur 15) pour un commun de niveau facile. Le tirage est un seul jet pondéré sur tout le groupe (`pickWeighted`) ; la chance réelle = poids ÷ total du groupe. L'admin modifie les poids objet par objet (`PUT /api/admin/drops { kind, weights }`, `null` = retour au défaut ; 0 = jamais), cache 60 s comme les règles. Les poids sont quantifiés à 0,001 % (largement au-dessus de la résolution du jet, 10⁻⁹), une valeur égale au défaut n'est pas stockée, le document stocké porte `unit: "percent"` (un document sans unité, écrit par la première version en millièmes, est lu divisé par 10), un groupe (niveau ou catalogue) entièrement à 0 est refusé (`empty_pool`, 400) et un document stocké partiellement invalide est récupéré entrée par entrée. Un troc accepté prend d'abord les deux exemplaires (décréments conditionnels) et rembourse / annule si l'un manque, avant de les redistribuer ; l'avis de cadeau reçu s'affiche sur tous les écrans « Créature » (œuf, incubation, deuil compris), et le badge compte exactement les cadeaux affichables.
- **Accès depuis la page de connexion** : le formulaire de connexion classique accepte aussi `ADMIN_USERNAME` / `ADMIN_PASSWORD` (identifiant sans `@` → tentative admin d'abord ; sinon après l'échec de la connexion joueur) et redirige vers `/admin`. L'identifiant admin est **réservé** : `isAdminIdentifier()` (`lib/admin/auth.ts`) bloque son usage comme pseudo (`createProfile`, `/api/profile/check`) et comme email ou nom à l'inscription (hook Better Auth `user.create.before`).

### 3.11 Jouer, accessoires, garde-robe, collection (phase 5)
- Mini-jeu (`components/game/food-catch-game.tsx`) : 20 s, des aliments (emoji) tombent, la créature suit le doigt (`pointermove`, easing), boucle `requestAnimationFrame` sans état React par frame (positions écrites dans `style.transform`, pool de 10 éléments réutilisés). Score = ratio d'aliments sains attrapés × 100 − 10 par malbouffe (`computePlayScore`, pur et testé). `POST /api/play` reçoit les compteurs bruts, recalcule le score côté serveur, applique `+15 humeur, +5 XP (+5 si parfait)`, limite `play.maxPerDay` parties / jour Paris (défaut 3, règle admin, `play_sessions`).
...
- **Exemplaires** : `user_accessories.qty` (migration 008). Un coffre qui tire un accessoire déjà possédé ajoute un exemplaire (plus d'XP de doublon) ; la garde-robe affiche « ×N ». `takeAccessoryCopy()` décrémente sous condition `qty > 0`, supprime la ligne et retire l'accessoire des tenues quand le dernier exemplaire part ; `addAccessoryCopies()` fait l'inverse. Les trocs déplacent un exemplaire dans chaque sens (on peut donc demander un accessoire qu'on possède déjà).
- **Dons** : `giftAccessory()` (`POST /api/friends/:id/gift { accessoryId }`) donne un exemplaire à un ami accepté, sans contrepartie ni acceptation ; une ligne `gifts` de `kind = 'accessory'` prévient le receveur sur son écran créature (encart « Un cadeau pour toi ») et par une pastille sur l'onglet Créature.
...
- **Confier** : `POST /api/friends/:id/board { days }` **propose** la créature vivante et nommée du joueur à un ami accepté (`boardings.status = pending`, migration 011) ; la créature reste chez lui tant que l'ami n'a pas répondu (`POST /api/boardings/:id/respond { accept }` : `active`, le séjour et sa durée partent de l'acceptation, capacité de l'hôte revérifiée ; ou `declined`). Le propriétaire peut retirer sa proposition (`cancelled`) ; une proposition dont la créature meurt entre-temps s'annule d'elle-même. L'hôte voit la proposition sur son écran Créature (pastille), le propriétaire est prévenu de la réponse et d'un retour anticipé (`owner_seen_at`, `POST /api/boardings/seen { role: "owner" }`). Durée de 1 jour au maximum du niveau (règle admin `tiers[tier].boardingMaxDays`, défaut 30 ; le dialogue propose 3 / 7 / 14 / 21 / 30 tronqués au maximum, `boardingLimits()`). Une seule pension ouverte (proposée ou active) par créature (index unique partiel `boardings_creature_open_idx`) ; `boarding.maxPerHost` (règle admin, défaut 5, 0 désactive) créatures hébergées au plus par joueur ; **repos** après une pension : si le dernier séjour du propriétaire a duré X (durée effective, quelle qu'en soit la fin), il attend X × `boarding.cooldownMultiplier` (règle admin, défaut 1, 0 = aucun repos) avant de confier à nouveau (`boardingCooldownUntil()`, erreur `boarding_cooldown`, note sur la page Amis). L'hôte est prévenu (pastille sur l'onglet Créature = pensions non vues, section « En pension chez toi » sur son écran d'accueil, `POST /api/boardings/seen` à l'ouverture).
...
- **Mort en pension** : la pension se ferme `died` à la date du décès ; le propriétaire retrouve l'écran de deuil (« pendant sa pension chez X ») puis choisit seul son nouvel œuf et son niveau ; l'hôte reçoit un encart « n'a pas survécu à sa pension chez toi » à acquitter (`host_seen_at` remis à `NULL` par la fermeture, `POST /api/boardings/seen { boardingId }`, compté dans la pastille) ; une pension terminée par un décès ne compte pas pour le repos.
...
### 3.17 Coaching
- **Relation** : `POST /api/coaching { friendshipId }` propose un ami accepté comme coach (`coachings`, statut `pending`) ; `POST /api/coaching/:id/respond { accept }` (le coach) → `active` ou `declined` ; `POST /api/coaching/:id/end` par l'un ou l'autre → `cancelled` (proposition retirée), `declined` ou `ended` (+ `ended_by`). Un seul coaching `pending`/`active` par élève (index unique partiel `coachings_student_open_idx`), plusieurs élèves par coach. Le coach est prévenu par la pastille de l'onglet Amis (propositions reçues), l'élève par un encart sur la page Coaching (`student_seen_at`, `POST /api/coaching/seen`).
- **Repas et pouces** : `/coach/:id` (coach uniquement, relation active) liste les repas de l'élève de la fenêtre de conservation avec photo signée (1 h), date et heure, note et verdict de l'appli, commentaire ; `POST /api/coaching/:id/meals/:mealId/review { verdict: up | down }` pose ou change le pouce (`meal_reviews`, un par repas). Les compteurs `thumbs_up` / `thumbs_down` sont tenus sur `coachings` (mise à jour par `UPDATE … + delta`) et restent visibles des deux côtés tant que la relation dure, même après la purge des repas. L'élève voit le pouce du coach dans son historique (`MealView.review`).
- **Récompenses** : élève = Σ max(verts − rouges, 0) sur ses coachings ÷ `coaching.thumbsPerStudentReward` (règle admin, défaut 5) ; coach = Σ (verts + rouges) ÷ `coaching.thumbsPerCoachReward` (défaut 10), tous élèves confondus. Ouverture `POST /api/coaching/rewards/open { role }` (réservation par `UPDATE profiles … WHERE x_rewards_opened = n`), tirage pondéré sur `rewardPool(role)` = accessoires des coffres + accessoires réservés au rôle (`Accessory.source`) : toque d'étudiant·e, cartable, médaille de progrès (élève) ; sifflet, casquette, fouet (coach). Ces six-là ne sortent jamais des coffres de pas. Même écran de révélation que les coffres (`ChestOpener mode="reward"`).
- **Conservation** : `purgeExpiredMeals()` supprime photo R2 puis ligne au-delà de `feeding.mealRetentionDays` (règle admin, défaut 30), appelée avant chaque lecture d'historique (page Repas, `GET /api/meals`, vue du coach) ; les avis suivent la suppression du repas (`ON DELETE CASCADE`), les compteurs non.

### 3.18 Effets de l'humeur
- **Bandes** (`lib/game/mood.ts`, seuils réglables depuis `/admin`, `rules.mood`) : *triste* sous `gloomyMax` (20), *morose* sous `lowMax` (30), *ravie* à partir de `happyMin` (70), *ça va* entre les deux. L'humeur elle-même bouge comme avant : `−moodLossPerHour` par heure selon le niveau, +5 par repas, +15 par partie.
- **XP** : tout gain d'XP (repas, partie, pas) est multiplié par `1 + xpBonusPercent/100` quand la créature est ravie (défaut ×1,25) et par `1 − xpMalusPercent/100` quand elle est morose ou triste (défaut ×0,75), arrondi, jamais 0 pour un gain positif sauf malus total. L'humeur prise en compte est celle **avant** l'action (`mealEffects({ mood })`, `playEffects(score, mood, rules)`, `applyStepGains(creature, gains, rules)`) ; chaque créature nourrie ou promenée applique son propre coefficient.
- **Santé** : sous `gloomyMax`, le tick retire `healthLossPerHourWhenGloomy` (défaut 0,5 pt/h) en plus de la faim, même nourrie (`gloomyHours()` calcule les heures passées sous le seuil pendant la décroissance).
- **Coffres** : quand des pas sont crédités à une créature ravie, `chestStepsBonusPercent` (défaut 10 %) de ces pas est ajouté à `creatures.chest_bonus_steps` (migration 012), que `getChestStatus()` additionne aux pas réels. Le bonus est acquis au moment du crédit : il ne clignote pas avec l'humeur.
- **Affichage** : légende de la jauge (« Ravie · XP +25 % », « Triste · santé −0,5 pt/h »), rappel de la règle sous les jauges (écrans Créature et Pension), coefficient rappelé sur le résultat d'un repas et d'une partie, réplique « J'ai le cœur lourd… tu viens jouer ? » quand triste.

### 3.19 Voir en vrai (réalité augmentée sur marqueur imprimé)
- **Écran** `/ar` (action « Voir en vrai » de l'écran Créature, entrée du menu Plus) : la caméra arrière du téléphone (`getUserMedia`, HTTPS) filme un marqueur carré imprimé ; la créature du joueur, avec ses accessoires, apparaît debout dessus. Réservé à sa propre créature vivante et nommée, chez soi (un œuf, une créature morte ou en pension affichent un message).
- **Un marqueur par créature** : numéro AprilTag 36h11 (587 codes, distance de Hamming 11 : une mauvaise lecture ne transforme jamais une créature en une autre) stocké dans `creatures.ar_marker` (migration 013) et attribué à la première demande (`ensureCreatureMarker()`), en évitant les numéros déjà pris par les créatures du propriétaire et de ses amis acceptés (les feuilles susceptibles d'être posées sur la même table). `GET /api/ar/marker?creature=<id>` (session, sa propre créature) renvoie une page A4 (pdf-lib) avec le carré de 8 cm, **le nom de la créature dessous**, la marge blanche, une ligne de découpe et les consignes ; la page `/ar` montre l'aperçu SVG (`markerSvg(id)`) avec le nom.
- **Plusieurs créatures à la fois** : `listArTargets()` donne au viewer tout ce que la caméra du joueur peut reconnaître : sa créature (même en pension ailleurs), celles qu'il héberge et celles de ses amis acceptés qui ont déjà un marqueur, une seule par numéro (priorité propre > hébergée > amis, les doublons sont signalés sur la page). Chaque marqueur reconnu porte sa créature avec une étiquette « nom · propriétaire » ; un ami pose sa propre feuille à côté et sa créature apparaît sur l'écran de tout le monde. Un marqueur inconnu du joueur (créature d'un inconnu) n'affiche rien. Le test `lib/ar/pdf.test.ts` relit les rectangles du PDF et vérifie qu'ils reproduisent exactement la grille du détecteur (`markerCells()`), dans le bon sens et à la bonne taille.
- **Détection** : js-aruco2 (MIT, intégré en modules ES dans `lib/ar/vendor/`, dictionnaire AprilTag BSD, chargé à la demande) analyse chaque image réduite à 640 px de large (une image sur deux quand la lecture dépasse 24 ms) ; chaque marqueur reconnu donne quatre coins, convertis en `MarkerPose` (`lib/ar/geometry.ts` : centre, taille apparente, angle dans le plan, inclinaison, étendues) avec un lissage exponentiel. Tout reste sur le téléphone : aucune image n'est envoyée.
- **Niveaux de rendu** (`components/ar/types.ts`) : niveau 1 (dépassé) = la créature plate toujours face à la caméra ; niveau 2 `ViewsRenderer` (**secours**) = faux 3D à **8 vues** (45° chacune) : la rotation du papier sur la table (`pose.angle`, angle du bord haut du marqueur à l'écran) choisit la vue avec une hystérésis de 6° (`viewFromAngle`), et le rendu passe `yaw = vue × 45°` au composant `Creature`. Aucun dessin supplémentaire : `turnPlacements()` projette les traits existants sur une sphère (`turnFeature`) : les yeux, joues, bouche et lunettes glissent vers le côté, se resserrent et disparaissent derrière la silhouette (un seul œil au profil, aucun trait de dos), les oreilles se décalent et se retournent, la queue tourne autour du corps et passe devant lui de dos, chapeau et collier suivent, la cape se voit de dos. **Niveau 3 « vraie 3D » (livré)** : un canvas WebGL recouvre la vidéo (même cadrage `object-fit: cover`) ; la pose complète de chaque marqueur (position et rotation, `lib/ar/pose3d.ts` : POSIT coplanaire de `lib/ar/vendor/posit1.js`, deux solutions miroir dont on garde la plus proche de l'image précédente, lissage) place un repère par papier dans lequel la créature se tient debout, tournée vers le bord bas (celui du nom) : on la voit de face, de profil, de dos ou de dessus selon l'angle de la feuille et du téléphone, et la perspective suit la caméra (champ de vision déduit d'une focale égale à la largeur de l'image). Les créatures sont des volumes construits automatiquement à partir des mêmes pièces que le dessin (`components/ar/three/creature-mesh.ts` : corps et tête en ellipsoïdes, ventre, yeux à reflet, bouche par type, joues, oreilles, queue, extras des rares : halo, ailes, cristaux, flammes, aura ; teintes malade / fatiguée / morte ; respiration, clignement et queue qui remue) dont les traits du visage et les marquages sont posés sur la surface du volume ; les accessoires sont leurs propres dessins rastérisés : chapeaux, lunettes et colliers en sprites toujours lisibles face à la caméra (cachés par le corps de dos), tenues et capes en cartes devant et derrière le corps. Hauteur ≈ 2,2 × le côté du marqueur, ombre au sol dans le plan du papier. Sans WebGL, le viewer retombe sur le niveau 2. Dans les deux cas, positions et étiquettes sont écrites chaque image hors de l'état React.
- **Photo** : le bouton « Photo » compose l'image de la caméra et toutes les créatures visibles (rendu 3D dessiné par-dessus ; en secours, SVG sérialisés, les plus lointaines d'abord) dans un canvas ; enregistrement (`<a download>`) ou partage natif (`navigator.share`) quand le navigateur le propose. La caméra est coupée quand l'onglet passe en arrière-plan.

### 3.20 Administration : onglet Joueurs
- `/admin/joueurs` liste tous les comptes ayant un profil (pseudo, code ami, email, date d'inscription), leur créature (l'œuf ou la créature vivante, sinon la dernière morte) avec ses statistiques du moment (santé, faim, humeur et effet d'humeur, XP et stade, âge, maladie et jours restants, talisman, coffres ouverts, pas bonus) et ses caractéristiques (espèce, rareté, niveau, accessoires portés), les compteurs du joueur (repas, pas, amis, créatures au cimetière) et sa situation de pension (créature confiée, créatures hébergées). Les statistiques sont calculées en mémoire (`applyTick`) sans écrire en base. Recherche par pseudo, email, code ami, nom de créature ou espèce, insensible aux accents.
- Bouton « Afficher le marqueur » : `GET /api/admin/marker?creature=<id>&format=svg` (aperçu) et `format=pdf` (impression), réservés à la session admin ; le numéro est attribué à la première ouverture comme pour le joueur, avec la même règle d'évitement.

### 3.21 Défendre (tower defense en réalité augmentée)
- **Écran** `/defense` (action « Défendre » de l'écran Créature, carte sur « Voir en vrai », entrée du menu Plus ; `?creature=<id>` pour une créature hébergée, dont le marqueur doit déjà exister) : la créature se tient sur son marqueur imprimé, la **malbouffe** surgit autour d'elle dans une bouffée de fumée et fonce dessus ; le joueur vise avec le centre de l'écran (un anneau montre où l'axe de la caméra touche le papier) et lance des œufs avec le bouton en bas à droite. Les points de vie perdus n'existent que dans le jeu.
- **Logique pure** `lib/game/defense.ts` (`createDefense`, `startDefense`, `stepDefense(state, dt, random)`, `fireDefense(state, aim)`, `endDefense`, tests à côté) dans le repère du papier (côté = 1, z vers le haut, créature à l'origine) : douze aliments (`JUNK_FOODS` : burger, pizza, hot-dog, frites, bonbon, chips, soda, glace, milkshake, donut, cookie, gâteau) avec chacun un **déplacement** (tout droit, zigzag, rebonds, spirale), des **dégâts** (10 à 25) et une **vague d'apparition** (1 à 5). Ils partent à 2,8 côtés ; une vague = `waveEnemyCount` aliments (5, +2 par vague) à `waveSpeed` (0,32 côté/s, +12 % par vague) avec un intervalle d'apparition qui se resserre ; un aliment touche la créature sous 0,45 côté. Un œuf part de la créature, qui se tourne vers la cible (`yawToward`), et détruit à l'atterrissage les aliments à moins de 0,5 côté et plus bas que 0,35 (les rebonds en l'air l'évitent) ; rechargement 350 ms ; score affiché = 10 × vague par aliment ; fin à 0 point de vie ou sur « Abandonner ». **Boss** : une vague sur trois (`bossEveryWaves`, réglable) contient un aliment géant (× 2,2) qui surgit à un moment aléatoire de la vague (créneau uniforme parmi ses apparitions, jamais prévisible), deux fois plus lent et deux fois plus dangereux, qui encaisse plusieurs œufs (3 pour le premier, un de plus à chaque boss suivant, `bossHits`) avec une **barre de vie** au-dessus de lui ; un œuf qui le touche fait un éclair sans le détruire ; il rapporte 10 × vague × œufs nécessaires. Les aliments ordinaires, détruits en un œuf, n'ont pas de barre. **Bons aliments et langue** : pendant une vague, des fruits et légumes en 3D (pomme, carotte, brocoli, banane, fraise, tomate) apparaissent sur la table (au plus deux, toutes les 6 à 10 s), y restent 4 s, clignotent 1,5 s puis disparaissent ; un œuf qui tombe dessus les écrase. Le bouton « Langue » (touche L) fait sortir la langue de la créature vers le point visé (jusqu'à 2,3 côtés) puis la rétracte : à pleine extension, tout ce qui se trouve dans son couloir (0,3 côté de large, sous 0,5 de hauteur) est attrapé et les effets s'additionnent, un bon aliment rend 6 à 12 points de vie (plafonnés au maximum), une malbouffe avalée par erreur inflige ses dégâts ; les boss sont trop gros pour la langue. Une langue à la fois, rechargement 0,9 s. Le générateur aléatoire est injecté : une partie se rejoue à l'identique.
- **Règles admin** (`rules.defense`, section « Défendre » de `/admin`) : points de vie, vitesse de base, accélération par vague (%), aliments de la première vague, aliments en plus par vague, rechargement, fréquence des boss (0 = jamais), œufs pour le premier boss.
- **Récompense** : `POST /api/defense { spawned, destroyed, reached, wavesCleared, creatureId? }` recalcule le score 0–100 (`computeDefenseScore` = part des aliments détruits, compteurs bornés par ce que les vagues atteintes ont pu envoyer, `maxSpawnedThrough`), « parfait » = tout détruit sur au moins 3 vagues, puis `recordPlay(…, "defense")` : mêmes effets que « Jouer » (+15 humeur, +5 XP, +5 si parfait, multiplicateur d'humeur) et **même limite** de `play.maxPerDay` parties par jour et par créature (défaut 3, règle admin) ; `play_sessions.kind` distingue les deux jeux (migration 014).
- **Rendu** : `MarkerCamera` (`components/ar/marker-camera.ts`, caméra et détecteur partagés avec « Voir en vrai ») → `ThreeStage` (pose du marqueur et créature ; `setYaw` la fait pivoter, `aimOnMarker` donne l'intersection de l'axe de la caméra avec le plan du papier) → `DefenseScene` (`components/ar/three/defense-scene.ts` : aliments en **volumes 3D** construits de primitives aux couleurs des dessins (`food-mesh.ts`, un modèle par sorte cloné par aliment, tournés vers la créature, ombre au sol), œufs en cloche, fumée, éclat de jaune, flash rouge, anneau de visée, objets recyclés). `DefenseGame` (`components/defense/defense-game.tsx`) tient la boucle : pas de simulation borné à 50 ms, **pause** quand le marqueur est perdu plus de 1,2 s ou quand l'onglet est caché (reprise possible), HUD (vague, points de vie, score) mis à jour seulement quand une valeur change, espace ou entrée tirent aussi, résultat puis envoi. Sans WebGL, le jeu explique qu'il ne peut pas se lancer.

### 3.22 Arène (bataille entre amis en réalité augmentée)
- **Écran** `/arena` (rangée « Voir en vrai · Arène » de l'écran Créature, carte sur « Voir en vrai », menu Plus) : invitations reçues, partie en cours, ouverture d'une partie avec 1 à 3 amis acceptés dont la créature est vivante et nommée, dernières batailles ; `/arena/:id` : salle d'attente (joueurs prêts / invités / déclinés, l'hôte lance dès qu'un ami a rejoint, chacun peut ouvrir sa caméra à l'avance), bataille, classement. Tous posent le marqueur de leur créature sur la même table ; chaque téléphone reconnaît tous les marqueurs de la partie et dessine chaque créature sur le sien, la santé des adversaires au-dessus de leur tête.
- **Règles** (`rules.arena`, section « Arène » de `/admin`, gelées dans la partie à sa création) : points de vie (100), dégâts par œuf (15), durée (180 s). Le centre de l'écran vise l'adversaire le plus proche du point visé (moins de 2,2 côtés) ; l'œuf part de sa créature (ou du téléphone quand sa propre feuille est hors champ) et touche sous 0,55 côté du centre de la cible, jugé à l'atterrissage dans le repère **actuel** de la feuille de la cible : déplacer sa feuille pendant le vol permet d'esquiver. Des fruits et légumes apparaissent sur la table à côté d'une créature debout (5 s après le début puis toutes les 9 s, au plus 3, 4,5 s chacun dont 1,5 s de clignotement) ; la langue (2,3 côtés au plus, une à la fois) rend leurs points de vie, plafonnés au maximum. Fin quand le temps est écoulé ou qu'une seule créature tient debout ; classement = survivants par santé puis touches, puis les autres dans l'ordre inverse de leur sortie ; quitter en cours = sortie immédiate, sans récompense. Récompense = `recordPlay(…, "arena")` avec un score 0–100 (moitié santé restante, moitié rang), « parfait » = premier avec toute sa santé ; **même limite quotidienne** que « Jouer » et « Défendre » (vérifiée à la création et à l'acceptation ; si elle est atteinte à la fin, la partie compte mais ne rapporte rien, `reward.skipped`). Une salle que personne ne lance est fermée après 30 min.
- **Arbitrage** : le serveur tient la vérité (santé, bons aliments, événements, fin, rangs, récompenses réservées par `UPDATE` conditionnel), calculée paresseusement à chaque lecture (`snapshot`). La géométrie (où tombe l'œuf, ce que balaie la langue) est jugée par le téléphone du tireur, seule sa caméra sachant où sont les feuilles ; le serveur impose la cadence (un œuf toutes les 250 ms au plus), les dégâts et le plafond de soin. Synchronisation par **sondage court** (`GET /api/arena/:id?since=` toutes les 500 ms en bataille, 2 s en salle, rien une fois la partie finie). Avec la case admin « WebRTC », les téléphones ouvrent en plus une **liaison directe** (canaux de données WebRTC en maillage complet, signalisation relayée par le serveur via `/api/arena/:id/signals`, serveurs STUN publics, pas de relais TURN) qui transmet les œufs (au lancer, puis le verdict à l'atterrissage) et les langues sans attendre ; le serveur reste l'arbitre, interrogé toutes les secondes ; chaque tir porte un identifiant (`nonce`) commun au message direct et à l'événement serveur, dessiné une seule fois ; une paire de téléphones qui ne parvient pas à se joindre reste sur le sondage ; case décochée, rien ne change.
- **Rendu** : `ArenaScene` (une couche par marqueur : barre de vie, langue, bons aliments ancrés, œufs, effets ; props partagés avec « Défendre »), `ThreeStage.worldPoint` / `localPoint` pour passer d'une feuille à l'autre, `ArenaGame` (caméra, boucle, HUD), `ArenaMatch` (salle et classement). Aperçu sans serveur : `/dev/screens?screen=arena` (arbitre en mémoire).
...
| `play_sessions` | Mini-jeux | `creature_id`, `user_id`, `score`, `kind ∈ {catch, defense, arena}` (migrations 014, 015) |
...
| `coachings` | Coaching (migration 010) | `student_id`, `coach_id`, `status` (`pending` / `active` / `declined` / `cancelled` / `ended`), `ended_by`, `thumbs_up`, `thumbs_down`, `student_seen_at` ; index unique partiel sur `student_id` tant que `status IN ('pending','active')` |
| `meal_reviews` | Pouce du coach sur un repas (migration 010) | PK `meal_id` (cascade), `coaching_id`, `verdict` (`up` / `down`) |
...
| `arena_matches` | Parties d'arène (migration 015) | `host_id`, `status` (`lobby` / `playing` / `finished` / `cancelled`), règles gelées `max_hp`, `egg_damage`, `duration_seconds`, `started_at`, `ends_at`, `finished_at`, `next_bonus_at` |
| `arena_players` | Joueurs d'une partie (migration 015) | `match_id`, `user_id`, `creature_id`, `marker`, `status` (`invited` / `ready` / `declined` / `left`), `hp`, compteurs (`hits_dealt`, `hits_taken`, `shots`, `good_eaten`, `healed`), `rank`, `eliminated_at`, `last_shot_at`, `rewarded_at`, `reward` jsonb ; unique `(match_id, user_id)` |
| `arena_bonuses` | Bons aliments posés sur la table (migration 015) | `match_id`, `anchor_user_id` (la créature à côté de laquelle il apparaît), `kind`, `x`, `y`, `heal`, `expires_at`, `eaten_by`, `eaten_at` |
| `arena_events` | Journal d'une partie (migration 015) | `id` bigserial (curseur des téléphones), `match_id`, `actor_id`, `kind` (`created` / `join` / `decline` / `start` / `leave` / `cancel` / `egg` / `tongue` / `eliminated` / `finish`), `payload` jsonb |

### 4.6 Animation de nourrissage
- Entre l'analyse et l'écran de résultat, `FeedFlow` passe par l'étape `serving` : `FeedAnimation` (`components/game/feed-animation.tsx`) dessine jusqu'à trois aliments reconnus (`foodKindsFor()` sur `meal.foods`, 101 types de dessins SVG (100 aliments + assiette) dans `components/food/food-icon.tsx`, repli « assiette »), les fait apparaître, puis les lance en arc vers chaque créature nourrie (la sienne et celles en pension chez soi) qui renifle, mange (`reaction eat`) ou grimace (`disgust` si la santé baisse). Au moment du repas, le gain ou la perte de santé flotte au-dessus de chaque créature (vert / rouge, `mm-delta-rise`) pendant que sa jauge et sa faim se mettent à jour ; la note du repas (`ScoreRing` + verdict) apparaît en dernier, puis l'écran de résultat détaillé. Durée ≈ 4,6 s, bouton « Passer », sautée d'office sous `prefers-reduced-motion`.
...
- `POST /api/defense { spawned, destroyed, reached, wavesCleared, creatureId? }` — score serveur + effets de « Défendre » (§ 3.21), même limite quotidienne que `/api/play`.
- `GET /api/arena` — `{ invitations, open, recent }` ; `POST /api/arena { friendIds }` — ouvre une salle d'arène (201, § 3.22).
- `GET /api/arena/:id[?since=<événement>]` — instantané complet (créatures incluses) ou, avec `since`, les événements après ce curseur ; réservé aux joueurs de la partie.
- `POST /api/arena/:id/action { action: join | decline | start | leave | cancel }` → instantané ; `{ action: "shoot", targetUserId, x, y, hit, nonce? }` → `{ accepted, hit, targetHp, eliminated }` ; `{ action: "eat", bonusIds, angle, length, nonce? }` → `{ eaten, healed, hp }`.
...
- Coaching : `POST /api/coaching { friendshipId }` ; `POST /api/coaching/:id/respond { accept }` ; `POST /api/coaching/:id/end` ; `POST /api/coaching/:id/meals/:mealId/review { verdict }` → `{ review, thumbsUp, thumbsDown }` ; `POST /api/coaching/rewards/open { role }` → récompense au format coffre ; `POST /api/coaching/seen`.
...
| D54 | La pension est une proposition que l'hôte accepte ou décline ; le séjour part de l'acceptation, et chacun peut y mettre fin à tout moment | Héberger engage (la créature peut mourir chez soi) : l'hôte choisit ; les propositions déclinées ou retirées ne comptent ni pour les coffres ni pour le repos. |
...
| D57 | Les compteurs de pouces vivent sur la relation, pas sur les repas | Les repas disparaissent après 30 jours ; le décompte doit survivre pour les récompenses et l'affichage des deux côtés. |
| D58 | Les récompenses de coaching se cumulent par joueur (tous coachings confondus) et se réservent par `UPDATE` conditionnel | Un coach de plusieurs élèves atteint le seuil, un coaching interrompu ne fait rien perdre, et deux ouvertures simultanées ne donnent qu'un accessoire. |
| D67 | Liaison WebRTC en maillage complet, signalée par le serveur sans trickle ICE, STUN public sans TURN, le serveur restant l'arbitre | Deux à quatre téléphones posés autour d'une même table sont presque toujours sur le même réseau : les canaux de données s'ouvrent sans relais et livrent les œufs en quelques dizaines de millisecondes au lieu d'un demi-sondage. Sans WebSocket, la signalisation passe par le journal de la partie (une offre et une réponse par paire, candidats inclus) ; garder l'arbitre côté serveur évite qu'un hôte tricheur décide des points de vie et des récompenses ; un identifiant par tir permet aux deux chemins (direct, serveur) de se recouvrir sans doublon, et le sondage reste le filet quand une paire ne se joint pas. |
| D66 | Arène : serveur arbitre joint par sondage court, géométrie jugée par le téléphone du tireur, œuf simulé dans le repère de la feuille du tireur | Sans WebSocket sur Vercel Hobby, un sondage toutes les 500 ms suffit pour 2 à 4 joueurs pendant 3 minutes et garde une seule source de vérité ; seule la caméra de chacun sait où sont les feuilles, le serveur ne peut donc que borner cadence, dégâts et soins ; ancrer le vol de l'œuf à la feuille du tireur (posée sur la table) le rend insensible aux mouvements du téléphone et permet à la cible d'esquiver en déplaçant la sienne. Le transport est une interface : le WebRTC viendra derrière la case admin déjà en place. |
| D65 | « Défendre » : logique pure dans le repère du papier, récompense calculée sur les compteurs, même limite quotidienne que « Jouer » | Le jeu se teste sans caméra ni WebGL (générateur injecté, parties rejouables) et la scène ne fait que dessiner l'état ; le serveur ne croit que des compteurs bornés par les vagues atteintes ; une seule limite évite de doubler l'XP du jour en alternant les deux jeux. |
| D64 | Vraie 3D par volumes construits à partir des pièces du dessin (Three.js) plutôt que des modèles glTF par espèce | 60 modèles à sculpter et à charger étaient hors de portée ; les sphères, cônes et tubes issus des mêmes gabarits (`LAYOUTS`) donnent une silhouette reconnaissable pour toutes les espèces et suivent automatiquement les stades, les états et les accessoires (cartes texturées de leurs propres SVG). POSIT, déjà intégré, fournit la pose sans bibliothèque de plus ; sans WebGL, les huit vues restent le secours. |
| D63 | Faux 3D par projection sphérique des traits existants plutôt que 8 dessins par espèce | 60 espèces × 4 stades × 8 vues à dessiner était hors de portée ; la projection donne des vues cohérentes pour toutes les espèces et tous les accessoires, à partir des pièces existantes, et `yaw = 0` reste pixel pour pixel le rendu d'origine. |
| D62 | Un marqueur AprilTag 36h11 par créature, attribué à la demande en évitant les numéros des amis | 587 codes très robustes suffisent : l'unicité n'a besoin d'être vraie qu'entre les feuilles posées sur la même table (soi, ses amis, ses pensionnaires), pas dans le monde entier ; en cas de doublon la page le dit et la créature du joueur passe en premier. |
| D61 | Réalité augmentée sur marqueur imprimé (AprilTag via js-aruco2 intégré) plutôt que WebXR | Le marqueur marche sur iPhone et Android dans une PWA sans installation ; WebXR n'existe que sur Android Chrome. La bibliothèque est intégrée en modules ES parce que son export via `this` ne survit pas au bundling. Le rendu est découplé (billboard aujourd'hui, vues multiples puis 3D ensuite) sans toucher à la détection. |
| D60 | Effets de l'humeur : XP ×1,25 / ×0,75, santé −0,5 pt/h sous 20, +10 % de pas de coffre, seuils admin | L'humeur n'avait aucun effet : le mini-jeu et les repas prennent du sens sans ajouter de cause de mort à elles seules (0,5 pt/h reste bien inférieur à la faim). Le bonus de coffre est acquis au crédit des pas pour rester déterministe. |
| D59 | Un accessoire garde un seul poids dans tous les groupes de tirage | L'admin règle une valeur par accessoire ; les groupes (coffres, récompenses élève, récompenses coach) ne diffèrent que par leur total, renormalisé au tirage. |
